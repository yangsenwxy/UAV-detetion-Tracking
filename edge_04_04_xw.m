...
clc;                %清除命令窗口
close all           %关闭所有窗口
avi=VideoReader('D:\KONGYU\se_2\test_file\20170401_clip.mp4');    %读取视频到avi
...
%         title('去掉过大过小值后剩余的面积'); 
       ...
       %%
           ...
           %将图像转换成一列
            picl4=bw4(:);
            %xw          
            fangchan=var(double(picl4));
            fangcha1=fangcha1+fangchan;
            %取出中值
            zhong4=median(picl4)
            if(zhong4<211)
            ...
            piclz4 = imadjust(bw4,[shangbian4 xiabian4],[]);%将图像中bw4的亮度值映射到J中的新值
    ...
    %         bw7(K_JINGQUE:(K_JINGQUE+2*JINGQUE_KUAN+1),K_JINGQUE:(K_JINGQUE+2*JINGQUE_KUAN+1))=bw7+piclz4;
          ...
          %%
           ...
           %将图像转换成一列
            ...
            if abs(SUB_GRAY_SUM-sub_gray_sum)<CHA1 && abs(fangcha1/i)<abs(fangchan)%追踪区相减后灰度均值相比
