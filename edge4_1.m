...
%膨胀参数
PENGZHANG=round((max((hang_end-hang_start),(lie_end-lie_start)))/6);
%膨胀结束
...
%         title('一列');
        ...
        if SUB_GRAY_SUM-sub_gray_sum<3  %追踪区相减后灰度均值相比
...
%         text(liezhong,hangzhong,'+','color','r');
    ...
    %     rectangle('position',status.BoundingBox,'edgecolor','r');
        ...
        if (((GRAY_SUM*1.8)>gray_sum)&&((GRAY_SUM*0.2)<gray_sum))
    ...
    if i==1
        ...
        if((length(pbb2)~=0)&&(YOUXIAO==1))
            ...
            if(abs(huitu_kuan_t*huitu_chang_t-HUITU_MIANJI)<(HUITU_MIANJI*0.2))
