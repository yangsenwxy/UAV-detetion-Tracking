...
t1=zeros(350,1);
HUITU_KUANGCHA=zeros(350,2);
HTHLC=zeros(350,2);
KUANG_MJC=zeros(350,1);
%手动画框的边界,封装后函数的输入变量
...
%             end
%xw             
                t=abs(SUB_GRAY_SUM-sub_gray_sum);
                t1(i,:)=t;
                t2=median(t1);
                t3=max(t1);
            if (t>t2&t<t3+1)%追踪区相减后灰度均值相比
    ...
    if i==1
        ...
        if((length(pbb2)~=0)&&(YOUXIAO==1))
           ...
           if(abs(huitu_kuan_t*huitu_chang_t-HUITU_MIANJI)<(HUITU_MIANJI*0.5))
             HUITU_KUANGCHA(i,:)=  [abs(HUITU_KUAN-huitu_kuan_t),abs(HUITU_CHANG-huitu_chang_t)]
             HTHLC(i,:)=[abs(HUITU_HANG-huitu_hang_t),abs(HUITU_LIE-huitu_lie_t)]
             KUANG_MJC(i,:)=abs((HUITU_KUANGCHA(i,1)+1)*(HUITU_KUANGCHA(i,2)+1))
             maxKUANG_MJC=max(KUANG_MJC(1:i-1))
           if (KUANG_MJC(i,:)<maxKUANG_MJC)
