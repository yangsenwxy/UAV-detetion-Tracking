...
clc;                %清除命令窗口
close all           %关闭所有窗口
avi=VideoReader('D:\KONGYU\se_2\test_file\20170329_飞行器.mp4');    %读取视频到avi
...
%         end
    ...
    detaX=abs(HUITU_LIE-last_x);
    detaY=abs(HUITU_HANG-last_y);
    if((detaY > 4)&&(detaX > 4)&&( i~=1))
        HUITU_LIE = last_x;
        HUITU_HANG = last_y;
    else   
        HUITU_LIE = HUITU_LIE;
        HUITU_HANG = HUITU_HANG;
    end 
      last_x  =HUITU_LIE;
      last_y = HUITU_HANG;   
