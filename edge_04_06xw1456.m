...
%xw             
 ...
 if (t>t2&t<t3+1)%追踪区相减后灰度均值相比若目标上帧不处于静止状态，正常执行 
...
%               end
    ...
    imshow(pixel(:, : ,i));
    if TAR_ON_HOLD==1
        rectangle('position',[HUITU_LIE-5,HUITU_HANG-5 HUITU_KUAN*2 HUITU_CHANG*2],'edgecolor','r');
    else
