...
clc;                %清除命令窗口
close all           %关闭所有窗口
avi=VideoReader('D:\KONGYU\se_2\yiceshi\2017040811.mp4');    %读取视频到avi
% t1=zeros(350,1); 
% t4=t1;
...
PIXEL_NUM_MAX=round(((((hang_end-hang_start)*(lie_end-lie_start))))*0.8);%识别目标的最大像素点个数
...
%                 if (abs(t1(i+1,:)-t1(i,:))
                ...
                t4=sum(t3(:))/(i-1)
               t5=abs(GRAY_AVR-gray_avr)               %小红框_4灰度均值差值比较
            %if ((t>t2)&&(t<t5))%追踪区相减后灰度均值相比xw
               if ((t<t2)||(t5>0))%追踪区相减后灰度均值相比
                %物体在运动
