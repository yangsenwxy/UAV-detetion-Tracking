...
clc;                %清除命令窗口
close all           %关闭所有窗口
avi=VideoReader('D:\KONGYU\se_2\yiceshi\xw0411.mp4');    %读取视频到avi
% t1=zeros(350,1); 
% t4=t1;
...
hang_start=266;  %xw0411，去掉前50帧，通过
hang_end=283;
lie_start=351;                                                                                                                                                                                                                                                                                    00;
lie_end=373;
% hang_start=256;%2017040811_人动相机动
% hang_end=276;
% lie_start=565;
% lie_end=580;
...
% lie_end=395;
% 
% hang_start=350;  %cs0409，去掉前50帧，通过
% hang_end=390;
% lie_start=225;                                                                                                                                                                                                                                                                                    00;
% lie_end=265;
...
KUANG_2=round(max((hang_end-hang_start),(lie_end-lie_start))*0.55);

% %精确追踪框的宽度
KUAN_3=round(KUANG_2*0.5);

K_JINGQUE=KUANG_2-KUAN_3;
...
PIXEL_NUM_MAX=round(((((hang_end-hang_start)*(lie_end-lie_start))))*0.55);%识别目标的最大像素点个数
...
for i=1:avi.NumberOfFrames-10 %循环提取每一帧
...
for i=1:nFrames-10
  pixel(:,:,i)=(rgb2gray(pixels(:,:,:,i+10)));  %RGB转灰度图像，存入pixel
end
...
%             title('差值图');
           ...
           %提高中心权限  
            ...
            %划定中心权限区,输入d，输出bw4
            bw4=d(hangzhong-KUAN_3:hangzhong+KUAN_3,liezhong-KUAN_3 :liezhong+KUAN_3,i);
            %划定追踪范围结束
    ...
    %         bw7(K_JINGQUE:(K_JINGQUE+2*KUAN_3+1),K_JINGQUE:(K_JINGQUE+2*KUAN_3+1))=bw7+piclz4;
            for z=K_JINGQUE:(K_JINGQUE+2*KUAN_3)
                for y=K_JINGQUE:(K_JINGQUE+2*KUAN_3)
...
%xw             
                t=abs(SUB_GRAY_AVR_LAST-sub_gray_avr); %KUANG_2的插值图灰度均值差值
                t1(i,:)=t%存到一个矩阵中
                t2=median(t1(:))  %取这个矩阵的中值
                t3(i,:)=t2                            %将这些中值放到另一个矩阵中再求个均值
                t4(i,:)=sum(t3(:))/i
                t5=max(t4)
               t6=abs(GRAY_AVR-gray_avr)               %小红框_4灰度均值差值比较
            %if ((t>t2)&&(t<t5))%追踪区相减后灰度均值相比
               if ((t<t5)||(t6>0))%追踪区相减后灰度均值相比
                %物体在运动
...
%%
        ...
        zx(i,:)=[liezhongy hangzhongx] 
        zxc(i,:)=abs(zx(i,:)-zx(i-1,:))
        maxzxc(i,:)=max(zxc)
        maxzxcavr=sum(maxzxc)/(i-1)
        if (zxc(i,:)<maxzxcavr)
    ...
    if(YOUXIAO==1)
        [l,m]=bwlabel(tf1);
        status=regionprops(l,'BoundingBox');%包含ROI的最小矩形
...
%         end
    ...
    if i==1
        if(length(pbb2)~=0)
            ...
            HUITU_MIANJI=HUITU_CHANG*HUITU_KUAN;%面积
        ...
        if((length(pbb2)~=0)&&(YOUXIAO==1))
            ...
            huitu_chang_t=pbb2(1,4);
             ...
             if(abs(huitu_kuan_t*huitu_chang_t-HUITU_MIANJI)<(HUITU_MIANJI*0.8))
              ...
              MAXKUANG_4MJC=max(KUAN_4_MJC(1:i-1))
            if (KUAN_4_MJC(i,:))<(MAXKUANG_4MJC)
