...
% kuang_4_cha=zeros(350,2);
% kuang_4_zbj=zeros(350,2);
% kuang_4_mjc=zeros(350,1);
%手动画框的边界,封装后函数的输入变量
...
KUANG_2_KUAN=round(max((hang_end-hang_start),(lie_end-lie_start))*0.5);

% %精确追踪框的宽度
JINGQUE_KUAN=round(KUANG_2_KUAN*0.5);

K_JINGQUE=KUANG_2_KUAN-JINGQUE_KUAN;
...
%         title('去掉过大过小值后剩余的面积'); 
       ...
       %%
           ...
           %将图像转换成一列
            ...
            %划定追踪范围,输入d，输出bw2
            bw2=d(hangzhong-KUANG_2_KUAN:hangzhong+KUANG_2_KUAN,liezhong-KUANG_2_KUAN :liezhong+KUANG_2_KUAN,i);
            bw5=bw2;
    ...
    %         bw7(K_JINGQUE:(K_JINGQUE+2*JINGQUE_KUAN+1),K_JINGQUE:(K_JINGQUE+2*JINGQUE_KUAN+1))=bw7+piclz4;
          ...
          %%
           ...
           %将图像转换成一列
            ...
            sub_gray_avr=sum(picl)/((KUANG_2_KUAN*2+1)*(KUANG_2_KUAN*2+1));
...
%%
        ...
        if(i==1)
            ...
            hangzhong=hangzhongx+hangzhong-KUANG_2_KUAN-1;
            liezhong=liezhongy+liezhong-KUANG_2_KUAN-1;
        end
        %还原真实坐标结束
...
%         end
    ...
    if i==1
        ...
        if((length(pbb2)~=0)&&(YOUXIAO==1))
            ...
            huitu_lie_t=liezhong-KUANG_2_KUAN+pbb2(1,1);
            huitu_hang_t=hangzhong-KUANG_2_KUAN+pbb2(1,2);
            huitu_kuan_t=pbb2(1,3);
            huitu_chang_t=pbb2(1,4);
    
           if(abs(huitu_kuan_t*huitu_chang_t-HUITU_MIANJI)<(HUITU_MIANJI*0.5))
             kuang_4_cha(i,:)= [abs(HUITU_KUAN-huitu_kuan_t),abs(HUITU_CHANG-huitu_chang_t)]
             kuang_4_zbj(i,:)=[abs(HUITU_HANG-huitu_hang_t),abs(HUITU_LIE-huitu_lie_t)]
             kuang_4_mjc(i,:)=abs((kuang_4_cha(i,1)+1)*(kuang_4_cha(i,2)+1))
             maxkuang_4_mjc(i,:)=max(kuang_4_mjc(i,:))
%              [n,r]=size(maxkuang_4_mjc(i,:))
%              t6=sum(maxkuang_4_mjc(i,:))/n
        %   if (kuang_4_mjc(i,:)<maxkuang_4_mjc(i,:))
