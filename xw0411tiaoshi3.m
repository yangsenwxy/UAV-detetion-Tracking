...
clc;                %清除命令窗口
close all           %关闭所有窗口
avi=VideoReader('D:\KONGYU\se_2\yiceshi\xw43.mp4');    %读取视频到avi
% t1=zeros(350,1); 
% t4=t1;
...
for i=1:avi.NumberOfFrames %循环提取每一帧
    ...
    %         bw7(K_JINGQUE:(K_JINGQUE+2*KUANG_3+1),K_JINGQUE:(K_JINGQUE+2*KUANG_3+1))=bw7+piclz4;
          ...
          %%
           ...
           %将图像转换成一列
            ...
            sub_gray_avr=sum(picl)/((KUANG_2*2)*(KUANG_2*2));
...
%         end
    ...
    if i==1
        ...
        if((length(pbb2)~=0)&&(YOUXIAO==1))
            ...
            huitu_kuan_t=pbb2(1,3)-0.5;
            huitu_chang_t=pbb2(1,4)-0.5;
                %改动411 
           if(abs(huitu_kuan_t*huitu_chang_t-HUITU_MIANJI)<(HUITU_MIANJI*0.5))
            ...
            if (KUAN_4_MJC(i,:))<(15)
                ...
                HUITU_KUAN=huitu_kuan_t-0.5;
                HUITU_CHANG=huitu_chang_t-0.5;
