...
clc;                %清除命令窗口
close all           %关闭所有窗口
avi=VideoReader('C:\Users\Administrator\Desktop\cs0409.mp4');    %读取视频到avi

%手动画框的边界,封装后函数的输入变量
% hang_start=270;   %新飞行器_格式_短,通过
% hang_end=300;
% lie_start=290;                                                                                                                                                                                                                                                                                    00;
% lie_end=320;

% hang_start=175;%飞行器_固定翼_格式化_1，通过
% hang_end=195;
% lie_start=250;                                                                                                                                                                                                                                                                                    00;
% lie_end=270;

hang_start=350;  %cs0409.mp4，通过
hang_end=390;
lie_start=225;                                                                                                                                                                                                                                                                                    00;
lie_end=265;
...
for i =1 : nFrames-10      
   %%
    if i==1      %判断是否为第一帧图像
         d(:, :, i) = (abs(pixel(:,:,i+5) - pixel(:,:,i)));   %求当前帧和下一帧的差，d为求出的差值图像
         d2(:, :, i) = (abs(pixel(:,:,i) - pixel(:,:,i+5)));
         d(:, :, i)=d(:, :, i)+d2(:, :, i);
...
%        dd=0;
%         while (dd==0)
%              ERZHIHUA=ERZHIHUA*0.5;
%              bw(:, :, i) = im2bw(piclz(:, :, i),ERZHIHUA);              %对差值图像二值化，得到的也就是最终用于处理的图像
% 
...
%         text(liezhongy,hangzhongx,'+','color','r');
        ...
        if(i==1)
            ...
            liezhong=liezhongy+liezhong-ZHUIZONG_KUAN-3;
        end
        %还原真实坐标结束
        %测试代码，删
%         figure(26);
...
%         text(liezhong,hangzhong,'+','color','r');
    ...
    %%  
    %精确追踪改进区
    mubiaoqu=pixel(hangzhong-JINGQUE_KUAN:hangzhong+JINGQUE_KUAN,liezhong-JINGQUE_KUAN:liezhong+JINGQUE_KUAN,i);
%     figure(21);
%     imshow(mubiaoqu);
%     title('精确追踪区');
    %%
